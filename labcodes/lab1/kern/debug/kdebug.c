 ...
 * the value to the place that stack pointer pointers to. And popping a value do the
 * opposite.
 *
...
print_stackframe(void) {
      ...
      *    (3.5) pop up a calling stack frame
      *           NOTICE: the calling funciton's return addr eip  = ss:[ebp+4]
      *                   the calling funciton's ebp = ss:[ebp]
      */

	uint32_t val_ebp = read_ebp();
	uint32_t val_eip = read_eip();
	int i = 0;
	int j = 0;
	for (; i < STACKFRAME_DEPTH && val_ebp != 0 ; i++) {
		cprintf("ebp:0x%08x eip:0x%08x args:", val_ebp, val_eip);
//		uint32_t *args = (uint32_t *)val_ebp + 2;
		uint32_t *args = ((uint32_t *)val_ebp)[2]; // parameters at ebp+2
		for (j = 0; j < 4; j ++) {
			cprintf("0x%08x ", args[j]);
		}
		cprintf("\n");
		print_debuginfo(val_eip-1);
		val_eip = ((uint32_t *)val_ebp)[1]; // return address
		val_ebp = ((uint32_t *)val_ebp)[0]; // ebp of last stack frame
	}
}
