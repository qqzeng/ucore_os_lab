    ...
    for (i = 0; i < memmap->nr_map; i ++) {
        ...
        if (memmap->map[i].type == E820_ARM) {
            ...
            if (begin < end) {
                ...
                if (begin < end) { // pa2page 获取起始 page 的地址
...
page_insert(pde_t *pgdir, struct Page *page, uintptr_t la, uint32_t perm) {
    pte_t *ptep = get_pte(pgdir, la, 1); // 1. 获取线性地址在此页目录中所对应的页表项
    if (ptep == NULL) {
        return -E_NO_MEM;
    }
    page_ref_inc(page); // 2. 更新此页面被索引的计数
    if (*ptep & PTE_P) { // 3. 若页表项所对应的物理页存在
        struct Page *p = pte2page(*ptep); // 3.1 获取页表项所包含的内容所对应的物理页
