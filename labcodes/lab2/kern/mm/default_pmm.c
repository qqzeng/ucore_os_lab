...
default_init_memmap(struct Page *base, size_t n) {
    ...
    nr_free += n;
//    list_add(&free_list, &(base->page_link)); // 添加此 page 块到双向链表中，表头的位置
...
//    	list_del(&(page->page_link));
        if (page->property > n) { // 对于余留下的 page 块
            struct Page *p = page + n;
            p->property = page->property - n;
            SetPageProperty(p);
//            list_add_after(&free_list, &(p->page_link));
            list_add_after(&(page->page_link), &(p->page_link)); // 将剩下的 page 块添加到，当前被移除的 page 块的后面
        }
        list_del(&(page->page_link));
        nr_free -= n;
...
default_free_pages(struct Page *base, size_t n) {
    ...
    while (le != &free_list) { // 以 base 为首或尾，循环合并相邻的 page 块
        p = le2page(le, page_link);
        le = list_next(le);
        if (base + base->property == p) { // 合并后面的 page 块
            base->property += p->property;
            ClearPageProperty(p);
            list_del(&(p->page_link));
        }
        else if (p + p->property == base) { // 合并前面的 page 块
    ...
    nr_free += n;
    // 将此 page 块添加到双向链表中，因为是 FFMA，因此添加到第一个地址比它大的 page 块的前面
    struct Page *page = NULL;
	le = &free_list;
//	int t = 0, k = 0;
	while ((le = list_next(le)) != &free_list) {
//		t++;
		...
		if (base + base->property <= page) {
			assert(base + base->property != page); // 若相等必然在前一个循环中已合并
//			k = 1;
			break;
		}
	}
//    list_add(&free_list, &(base->page_link)); // 不能直接将此 page 块添加到双向链表的末尾
//	list_add_before(&(page->page_link), &(base->page_link)); // page->page_link 可能并未被赋予正确的值
//	cprintf("==%d==%d==%d==%d==%d==\n", le, le == &(page->page_link), &(page->page_link), t, k);
